...
| `_refresh_sessions(storage)` | 调用 `storage.list_sessions()` 获取按 session 分组的备份记录，填入左栏 `session_table` |
| `_on_session_selected()` | 左栏选中事件：按 `session_id` 从 `_session_data` 中取出对应版本，填入右栏 `detail_table` |
| `_restore_selected()` | 恢复右栏中选中的单个配置版本 |
| `_restore_session()` | 恢复当前选中 session 中的所有配置（依次逐个执行 RestoreWorker） |
| `_restore_next_in_session()` | 批次恢复的递归回调，处理队列中下一个版本 |
| `_restore_done(result)` | 单配置恢复完成，如果批次队列未空则调用 `_restore_next_in_session`，否则弹出完成对话框 |
| `_restore_error(msg)` | 恢复失败时启用按钮，弹出错误对话框，清空批次队列 |
| `_handle_blocked(msg, procs)` | 文件被占用时启用按钮，清空批次队列，弹出提示建议关闭指定程序 |
...
- ⚠️ **备份历史双栏**：左栏 `session_table` 显示备份批次（按 `session_id` 聚合），右栏 `detail_table` 显示选中批次中包含的各个配置。底部「恢复此批次」按钮依次恢复所有配置。
