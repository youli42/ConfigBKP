...
| `src/gui/config_tab.py` | JSONC 源码编辑器 + 实时语法验证 + 新建/删除规则 | PySide6, json5 |
...
| 文档 | 内容 |
|------|------|
| `01-程序入口与权限.md` | main.py：UAC 提权 + 启动逻辑 + logging 配置 |
| `02-核心逻辑层.md` | config_parser / backup_engine / restore_engine / manifest_manager / scanner |
| `03-存储后端.md` | base / local / zip_storage |
| `04-界面层.md` | main_window / home_tab / config_tab / settings_tab |
| `05-工具层.md` | app_path / path_expander / file_hasher / time_util / file_locker |
| `06-配置文件格式与书写指南.md` | JSONC 格式说明 + 面向用户的书写指南（面向用户） |
| `07-数据模型.md` | BackupResult / RestoreResult / BackupVersion / BackupSummary / 信号类 |
| `08-调试模式使用指南.md` | 环境变量设置 / 日志级别 / 排查方法 |
