...
| 文档 | 内容 |
|------|------|
| `代码文档/01-程序入口与权限.md` | main.py + cli.py：UAC 提权 + --silent-backup + logging 配置 |
...
| `代码文档/05-工具层.md` | app_path / path_expander / file_hasher / time_util / i18n / file_locker |
...
| `i18n/多语言贡献方法.md` | 多语言国际化：翻译贡献指南 / tr() 用法 / 文件结构 |

## 模块概览

| 文件 | 核心职责 | 外部依赖（非标准库） |
|------|----------|----------------------|
| `main.py` | 入口：argparse 解析参数（`--silent-backup`），UAC 提权，启动 GUI 或 CLI | PySide6 |
| `src/cli.py` | `run_silent_backup()`：无 GUI 静默备份模式，通过 QCoreApplication 运行 BatchBackupWorker | PySide6 |
...
| `src/core/restore_engine.py` | 通过 QRunnable 在后台线程执行恢复；支持 `config`/`data` 双路径路由；`source_type` 驱动目标匹配 | PySide6 |
...
| `src/gui/home_tab.py` | QTableWidget 配置列表（☑/名称/描述/范围） + 备份/恢复 + 按 `backup_scope` 分别收集 config/data + 历史双栏 | PySide6 |
| `src/gui/config_tab.py` | QTableWidget 规则列表（名称+version/启用开关/描述） + 源码/向导双模式 | PySide6, json5 |
| `src/gui/config_wizard.py` | 5 步向导表单（基本信息/源路径/解析字段/备份策略/数据路径） | PySide6 |
...
| `src/utils/file_utils.py` | `collect_files()` 展开路径收集文件；`filter_ignored()` 按 glob 模式过滤 | fnmatch |
| `src/utils/time_util.py` | 将 UTC 时间戳转换为本地时间，用于 UI 展示 | — |
| `src/utils/i18n.py` | 多语言支持：`tr()` 翻译 + `install()` 语言切换 + QSettings 持久化 | PySide6 |
| `lang/*.ts` / `lang/*.qm` | Qt 翻译文件，由 `i18n.py` 加载；`extract_strings.py` / `fill_translations.py` 辅助工具 | — |
