...
抽象基类，定义了 7 个抽象方法。`LocalStorage` 和 `ZipStorage` 分别实现。

| 方法 | 参数 | 返回值 |
|------|------|--------|
| `save` | `backup_id, config_name, files, note, description, session_id="", source_types=None` | `BackupResult` |
| `list_versions` | `config_name: str` | `list[BackupVersion]` |
| `list_sessions` | 无 | `list[BackupSession]` |
| `read_meta` | `config_name: str, backup_id: str` | `dict` |
