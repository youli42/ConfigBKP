...
文件职责：多语言支持模块。提供统一的 `tr()` 翻译函数和 `install()` 语言切换，使用 Python dict 存储翻译键值对（非 Qt QTranslator），零构建步骤。

### 公开接口

| 函数 | 参数 | 返回值 | 核心作用 |
|------|------|--------|----------|
| `install(locale)` | `locale: str`（如 `"zh_CN"` / `"en_US"`） | `None` | 加载对应语言的 `lang.{locale}.DATA` 字典到 `_translations` |
| `tr(key)` | `key: str` | `str` | 返回当前语言的翻译，无翻译时返回原文 |
| `current_locale()` | 无 | `str` | 返回当前语言代码 |
...
button = QPushButton(tr("备份"))
msg = tr("发现 {} 个文件").format(n)
```
...
- 中文为源语言（`lang/zh_CN.py` 内容为空），无需翻译。
- `main.py` 启动时从 `QSettings` 读取 `language` key 并 `install()`。
- 设置 Tab 中「语言」下拉框可切换语言，写入 QSettings 后提示重启生效。
- `build.ps1` 在打包时自动将 `lang/` 目录复制到 exe 同目录。
