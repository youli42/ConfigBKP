...
| `resolve_path_for_platform(cfg, key, system)` | `cfg: dict`, `key: str`, `system: str = ""` | `list[str]` | 取配置中 `key`（如 `paths`）的值，若为 dict 则按当前平台路由，若为 list 则原样返回 | 缺少当前平台键时回退 `default` 或空列表 |
...
- ⚠️ **路径遍历保护**：当 `restore_dir` 不为 None 时，目标路径经 `resolve()` 后与 `restore_dir` 比较，防止 `rel_path` 中的 `..` 越界写入。
- 恢复前使用 `tempfile.mkdtemp(prefix="restore_undo_")` 备份当前目标文件，恢复完成后 `shutil.rmtree` 清理。被占用或失败时同样清理。
- 映射目标路径时：若 `restore_dir` 不为 None，则恢复至该目录下；否则根据 `source_type` 选择 `paths`（config）或 `data_paths`（data）中的源路径（通过 `path_expander.expand` 展开）。`source_types` 通过 `StorageBackend.read_meta()` 从 metadata 读取。
