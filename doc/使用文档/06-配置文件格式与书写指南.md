...
| `paths` | `string[]` 或 `object` | 是 | 要备份的文件或目录路径列表，支持 `%xxx%` 环境变量。可以为数组（所有平台共用），或对象（按平台分配，详见下方多平台路径说明）。每条路径可选附加 `desc` 描述字段 |
...
每条路径可附加描述信息，便于在 GUI 中区分不同路径的用途：

```jsonc
"paths": {
  "windows": [
    {"path": "%APPDATA%\\Code\\User\\settings.json", "desc": "VS Code 用户设置"},
    {"path": "%APPDATA%\\Code\\User\\keybindings.json", "desc": "快捷键配置"},
    "%APPDATA%\\Code\\User\\snippets\\"   // 纯字符串 = 无描述，兼容旧格式
  ]
}
```

描述在向导路径列表中显示为第一列，鼠标悬停显示全文。纯字符串格式的路径自动转为 `{"path": "...", "desc": ""}`。
