...
| 功能 | 状态 | 说明 |
|------|------|------|
| 注册表备份 | 待开发 | 导出/恢复 Windows 注册表配置项，支持指定注册表路径 |
| 远程存储 (FTP/SFTP/WebDAV) | 待开发 | 备份到远程服务器，凭证加密存储，GUI 连接配置面板 |
| 手动排序规则 | 待开发 | 主页拖拽调整配置规则顺序 |
| 备份对比 Diff | 待开发 | 选中两个备份版本，展示文件差异列表 |
| 批量导入/导出规则 | 待开发 | 一键导出全部配置规则为 zip，另一台机器导入 |
| 静默模式完善 | 待开发 | `--silent-backup` 支持指定目标路径、指定规则子集 |
| 自定义通知 | 待开发 | 备份完成后弹 Windows Toast 通知 |
